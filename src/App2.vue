    ...
    <v-content>
      <v-container fluid class="wrapper">
        <v-row>
          <v-col>
            <div class="list">
              <drag v-for="n in [1,2,3,4,5]" :data="n" :key="n" class="item" type="test">{{n}}</drag>
            </div>
          </v-col>
          <v-col>
            <drop-list :items="items" class="list" @insert="onInsert" style="min-height: 100px" @reorder="$event.apply(items)" accepts-type="test">
              <template v-slot:item="{item, reorder}">
                <drag class="item" :key="item" :drag-image-opacity="1" @cut="items = items.filter(i => i !== item)" :style="{opacity: reorder ? 0 : 1}">
                  {{item}}
                </drag>
              </template>
