<template>
  <v-app>
    <v-content>
          ...
          <v-row align-content="stretch">
            <v-col>
              ...
              <v-list three-line class="list1">
                <drop-list :items="items1" :scrolling-edge-size="0" mode="cut" @reorder="$event.apply(items1)" @insert="insert1">
                  <template v-slot:item="{item, reorder, index}">
                    <drag :key="`item-${item}`" :data="item" @cut="remove(items1, item)" :delay="500" :vibration="5">
                      <v-list-item style="background-color: white; user-select: none" :style="reorder ? {borderLeft: '2px solid #1976D2', marginLeft:'-2px'} : {}">
                        <v-list-item-avatar>
                          {{ index }}
                        </v-list-item-avatar>
                        <v-list-item-content>
                  ...
                  <template v-slot:feedback="{data}">
                    <v-skeleton-loader type="list-item-avatar-three-line" :key="data" style="border-left: 2px solid #1976D2; margin-left: -2px;" />
                  </template>
            ...
            <v-col>
              <drop-list class="list2" :items="items2" @reorder="$event.apply(items2)" @insert="insert2" mode="cut">
                <template v-slot:item="{item,reorder,index}">
      ...
      <drop-list class="overlapper" :items="items1" :scrolling-edge-size="0" mode="cut" @reorder="$event.apply(items1)" @insert="insert1">
        <template v-slot:item="{item, reorder, index}">
